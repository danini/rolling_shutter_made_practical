...
// Rolling-shutter essential matrix from affine correspondences (Nx12, normalised coords).
// Returns: (model_8x3, inlier_indices, score, iterations)
std::tuple<Eigen::MatrixXd, std::vector<size_t>, double, size_t> estimateRSEssentialMatrix(
...
std::tuple<Eigen::MatrixXd, std::vector<size_t>, double, size_t> estimateRSEssentialMatrix7AC(
    const DataMatrix& kCorrespondences_,
    const double kFyOverH_,
    const std::vector<double>& kPointProbabilities_,
    const std::vector<double>& kImageSizes_,
    superansac::RANSACSettings& settings_,
    const double kACWeight_ = 1.0,
    const std::string& kNonMinSolver_ = "pc");

// Generic RS baseline solver (pc_rs, npt_lin, npt_gn, npt_cv, npt_iter)
std::tuple<Eigen::MatrixXd, std::vector<size_t>, double, size_t> estimateRSEssentialMatrixGeneric(
    const DataMatrix& kCorrespondences_,
...
PYBIND11_MODULE(pysuperansac, m) {
    ...
    // Rolling-shutter essential matrix from affine correspondences.
    m.def(
        ...
        [](py::array_t<double, py::array::c_style> correspondences,
           ...
           superansac::RANSACSettings& config) {
            ...
            py::gil_scoped_release release;
            return estimateRSEssentialMatrix(mat, fy_over_h, prob_vec, img_vec, config);
        },
        "Estimate the rolling-shutter essential matrix from affine correspondences.\n"
        "correspondences: Nx12 float64 array [x1,y1,x2,y2,tau1,tau2,J00,J10,J20,J01,J11,J21]\n"
        "                 (normalised image coordinates, tau in [-0.5,0.5])\n"
        "fy_over_h: fy / image_height (e.g. 743/1024 for TUM-RS)\n"
        "Returns: (model_8x3, inlier_indices, score, iterations)",
        py::arg("correspondences"),
        py::arg("fy_over_h"),
        py::arg("image_sizes"),
    ...
    // 7-AC Direct (action matrix) variant — sample size 7, debug prints active.
    m.def(
        ...
        [](py::array_t<double, py::array::c_style> correspondences,
           ...
           std::string nonmin_solver) {
            ...
            py::gil_scoped_release release;
            return estimateRSEssentialMatrix7AC(mat, fy_over_h, prob_vec, img_vec, config, ac_weight, nonmin_solver);
        },
        "Estimate the RS essential matrix using the 7-AC direct (action matrix) solver.\n"
        "Requires at least 7 affine correspondences.\n"
        "Returns: (model_8x3, inlier_indices, score, iterations)",
        py::arg("correspondences"),
        py::arg("fy_over_h"),
        py::arg("image_sizes"),
        py::arg("probabilities") = py::none(),
        py::arg("config") = superansac::RANSACSettings(),
        py::arg("ac_weight") = 1.0,
        py::arg("nonmin_solver") = "pc");
    ...
    m.def(
        ...
        [](py::array_t<double, py::array::c_style> correspondences,
           ...
           superansac::RANSACSettings& config) {
            ...
            py::gil_scoped_release release;
            return estimateRSEssentialMatrixGeneric(mat, method, fy_over_h, prob_vec, img_vec, config);
        },
        "Estimate the RS essential matrix using a baseline method.\n"
        "method: one of 'pc_rs', 'npt_lin', 'npt_gn', 'npt_cv', 'npt_iter'\n"
        "Returns: (model_8x3, inlier_indices, score, iterations)",
        py::arg("correspondences"),
        py::arg("method"),
        py::arg("fy_over_h"),
